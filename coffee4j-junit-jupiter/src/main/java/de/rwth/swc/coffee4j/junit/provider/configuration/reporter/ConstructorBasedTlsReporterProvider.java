...
//This is a modified copy of coffee4j's ConstructorBasedReporterProvider.
class ConstructorBasedTlsReporterProvider implements ReporterProvider, AnnotationConsumer<TlsReporter> {
    ...
    private ExecutionReporter instantiateReporter(Class<? extends ExecutionReporter> reporterClass) {
        try {
            return reporterClass.getConstructor(ExtensionContext.class).newInstance(extensionContext);
        } catch (Exception e) {
